...
void sendUserInfoToDB(User *user) {
    ...
    sprintf(query, "SELECT id FROM User WHERE nickname = '%s'", user->nickname);
    if (mysql_query(conn, query)) {
        fprintf(stderr, "%s\n", mysql_error(conn));
        exit(1);
    }
    ...
    for (i = 0; i < user->nb_characters; i++) {
        ...
        sprintf(query, "SELECT LAST id FROM Weapons WHERE name = '%s'", gearWeapon->name);
        if (mysql_query(conn, query)) {
            fprintf(stderr, "%s\n", mysql_error(conn));
            exit(1);
        }
        ...
        sprintf(query, "SELECT LAST id FROM Armors WHERE name = '%s'", gearArmor->name);
        if (mysql_query(conn, query)) {
            fprintf(stderr, "%s\n", mysql_error(conn));
            exit(1);
        }
        ...
        sprintf(query,
                "INSERT INTO Character (class_id, className, physicalPower, magicalPower, maxHp, currentHp, maxMp, currentMp, level, experience, experienceNeededToLevelUp, element, isAlive, user_id, gear_weapon_id, gear_armor_id) VALUES ('%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
                ...
                gear_weapon_id, gear_armor_id);
        if (mysql_query(conn, query)) {
            fprintf(stderr, "%s\n", mysql_error(conn));
            exit(1);
        }
        /* Get the character_id generated by the previous query */
        sprintf(query, "SELECT LAST id FROM Characters WHERE user_id = '%s'", user->id);
        if (mysql_query(conn, query)) {
            fprintf(stderr, "%s\n", mysql_error(conn));
            exit(1);
        }
...
void getDataFromDatabase(User *user){
    ...
    /* Get user data */
    sprintf(query, "SELECT * FROM Users WHERE id = '%s'", user->id);
    if (mysql_query(conn, query)) {
        fprintf(stderr, "%s\n", mysql_error(conn));
        exit(1);
    }
